...
class ViTBackbone(nn.Module):
    ...
    def forward(self, x):
        # x is the hidden state (smashed data) from Head or previous Backbone
        x = self.transformer(x)
        return x


class ViTTail(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, num_classes, dim_head=64, dropout=0., pool='cls'):
        super().__init__()
        self.transformer = Transformer(
            dim, depth, heads, dim_head, mlp_dim, dropout)
        self.pool = pool
